  ...
  renderNode = (node, parentNodes, isRoot = false) => {
    ...
    if (children.length === 0 || node.type === 'list_item') {
      ...
      for (let a = parentNodes.length - 1; a > -1; a--) {
        ...
        let refStyle = {};
        if (parentNodes[a].attributes && parentNodes[a].attributes.style && typeof parentNodes[a].attributes.style === 'string') {
          refStyle = (0, _convertAdditionalStyles.default)(parentNodes[a].attributes.style);
        }
        ...
        if (this._style[parentNodes[a].type]) {
          refStyle = {
            ...refStyle,
            ..._reactNative.StyleSheet.flatten(this._style[parentNodes[a].type])
          };
    ...
    if (isRoot === true && this._maxTopLevelChildren && children.length > this._maxTopLevelChildren) {
